...
namespace ConsoleApp22
{
    internal class Program
    {
            ...
            while (isActive)
            {
                ...
                switch (userInput)
                {
                    case AddDossierCommand:
                        AddDossier(ref fullNames, ref workPositions, extensionArray, messageTextInputName, messageTextInputPosition);
                        break;
                    ...
                    case DeleteDossierCommand:
                        DeleteDossier(ref fullNames, ref workPositions, extensionArray, messageTextDeleteDossier);
                        break;

                    case SearchBySurnameCommand:
                        FindDossierBySurname(fullNames, extensionArray);
                        break;
        ...
        static void AddDossier(ref string[] names, ref string[] positions, int extensionArray, string messageTextNames, string messageTextPosition)
        {
            Console.Clear();
            names = ResizeArray(names, extensionArray, messageTextNames);
            positions = ResizeArray(positions, extensionArray, messageTextPosition);
        }

        static string[] ResizeArray(string[] array, int extensionArray, string messageTextNames)
        {
        ...
        static void DeleteDossier(ref string[] fullNameArray, ref string[] workPositionArray, int extensionArray, string messageTextDeleteDossier)
        {
