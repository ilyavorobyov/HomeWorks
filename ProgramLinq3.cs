...
namespace TopPlayersLinq
{
    ...
    class PlayersManager
    {
        ...
        private void ShowTopThreeByLevel()
        {
            var topThreePlayerseList = _players.OrderByDescending(player => player.Level).Take(3).ToList();
            ShowList(topThreePlayerseList);
        }

        private void ShowTopThreeByPower()
        {
            var topThreePlayerseList = _players.OrderByDescending(player => player.Power).Take(3).ToList();
            ShowList(topThreePlayerseList);
        }
