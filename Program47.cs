...
namespace war
{
    ...
    class BattleField
    {
        ...
        {
            while (_redCountryPlatoon.GetNumberOfSoldiers() > 0 && _blueCountryPlatoon.GetNumberOfSoldiers() > 0)
            {
                ...
                _redCountryPlatoon.CheckHealthOfSoldier(_firstSoldier);
                _secondSoldier.Takedamage(_firstSoldier.Damage);
                _blueCountryPlatoon.CheckHealthOfSoldier(_secondSoldier);
        ...
        {
            if (_redCountryPlatoon.GetNumberOfSoldiers() < 0 && _blueCountryPlatoon.GetNumberOfSoldiers() < 0)
            {
                Console.WriteLine("Ничья все погибли");
            }
            else if (_redCountryPlatoon.GetNumberOfSoldiers() <= 0)
            {
                Console.WriteLine("Победил взвод второй страны");
            }
            else if (_blueCountryPlatoon.GetNumberOfSoldiers() <= 0)
            {
    ...
    class Platoon
    {
        ...
        public void CheckHealthOfSoldier(Soldier soldier)
        {
            if (soldier.Health <= 0)
            {
