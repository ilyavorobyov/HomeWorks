...
namespace RefactoringNapilnik2
{
    public class Model
    {
        ...
        {
            _connectionString = string.Format("Data Source=" + Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) + "\\db.sqlite");

            try
            {
                TryConnect(commandText);
            }
        ...
        public void TryConnect(string commandText)
        {
    ...
    public class View
    {
        ...
        {
            if (this._passportTextbox.Text.Trim() == "")
            {
            ...
            else
            {
                ...
                if (_rawData.Length < 10)
                {
                    this.textResult.Text = InvalidPassportInfo;
                }
                else
                {
                    CommandText = string.Format("select * from passports where num='{0}' limit 1;", (object)Form1.ComputeSha256Hash(_rawData));
