...
namespace ConsoleApp22
{
    internal class Program
    {
            ...
            while (isActive)
            {
                ...
                switch (userInput)
                {
                    ...
                    case DeleteDossierCommand:
                        DeleteDossier(ref employees, messageTextDeleteDossier);
                        break;
        ...
        static void ShowAllDossier(Dictionary<string, string> employees, string spaceForShow)
        {
            ...
            foreach (var column in employees)
            {
                Console.WriteLine(column.Key + spaceForShow + column.Value);
            }
        ...
        static void DeleteDossier(ref Dictionary<string, string> employees, string messageTextDeleteDossier)
        {
            int counter = 0;
            ...
            {
                foreach (var column in employees.Keys)
                {
                    counter++;

                    if (counter == numberOfDeletedDossier)
                    {
                        employees.Remove(column);
                        break;
                    }
                }

                Console.WriteLine("Удаление записи успешно завершено! Нажми любую кнопку.");
                Console.WriteLine(counter + " " + numberOfDeletedDossier);
                Console.ReadKey();
            }
        }
    }
