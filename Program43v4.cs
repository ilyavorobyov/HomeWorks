...
namespace trainconfig
{
    ...
    class StationManager
    {
        ...
        private void CalculateNumberOfCarriages(int soldTickets, string newRoute)
        {
            ...
            AddTrain(requiredNumberOfRailwayWagons, newRoute);
        }

        private void AddTrain(int requiredNumberOfRailwayWagons, string route)
        {
            _trains.Add(new Train(route, requiredNumberOfRailwayWagons));
        }
    }
    ...
    class Train
    {
        ...
        public Train(string route, int requiredNumberOfRailwayWagons)
        {
            _route = route;
            AddCarriages(requiredNumberOfRailwayWagons);
        }

        public void ShowInfo()
        {
            Console.WriteLine($"Поезд маршрута {_route}, потребовалось вагонов: {_carriages.Count}");
        }

        private void AddCarriages(int requiredNumberOfRailwayWagons)
        {
            for (int i = 0; i < requiredNumberOfRailwayWagons; i++)
            {
